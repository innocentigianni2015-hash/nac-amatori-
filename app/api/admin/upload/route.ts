...
export async function POST(request:Request) {
  ...
  const entity=cleanText(form.get("entity"),20);
  const id=cleanText(form.get("id"),100);
  if(!(file instanceof File)||!id||!["player","staff","sponsor","setting"].includes(entity)) return Response.json({error:"File non valido"},{status:400});
  ...
  if(entity==="setting"){
    ...
    const previous=await db.prepare("SELECT value FROM site_settings WHERE key=?").bind(settingKey).first<{value:string}>();
    ...
    if(previous?.value) await env.BUCKET.delete(previous.value);
