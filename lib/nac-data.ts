...
export async function ensureFirstAdmin(email:string,name:string){const db=getD1();const count=await db.prepare("SELECT COUNT(*) AS total FROM admins").first<{total:number}>();if(Number(count?.total||0)===0){await db.prepare("INSERT OR IGNORE INTO admins (email, name, role) VALUES (?, ?, 'owner')").bind(email.toLowerCase(),name).run();}const row=await db.prepare("SELECT email FROM admins WHERE email = ?").bind(email.toLowerCase()).first();return Boolean(row);}
export async function requireAdminApi(){const user=await getChatGPTUser();if(!user)return {user:null,error:Response.json({error:"Accesso richiesto"},{status:401})};const row=await getD1().prepare("SELECT email FROM admins WHERE email = ?").bind(user.email.toLowerCase()).first();if(!row)return {user,error:Response.json({error:"Utente non autorizzato"},{status:403})};return {user,error:null};}
...
export async function getAdminSnapshot(){
  ...
  return {players:players.results,events:events.results,attendance:attendance.results,votes:votes.results,messages:messages.results,sponsors:sponsors.results,settings:Object.fromEntries(settings.results.map(x=>[x.key,x.value])),matches:matches.results,standings:calculateStandings(matches.results)};
}
