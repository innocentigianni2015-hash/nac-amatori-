...
export type ArticleRow={id:string;title:string;description:string;body:string;imageKey:string|null;status:string;authorEmail:string;publishedAt:string|null;createdAt:string;updatedAt:string};
...
export async function getAdminSnapshot(currentEmail?:string){
  ...
  const [players,staff,events,attendance,votes,messages,sponsors,settings,matches,articles,admins]=await Promise.all([
    ...
    db.prepare("SELECT id,title,description,body,image_key AS imageKey,status,author_email AS authorEmail,published_at AS publishedAt,created_at AS createdAt,updated_at AS updatedAt FROM articles ORDER BY created_at DESC").all<ArticleRow>(),
...
export async function getPublishedArticles(){const r=await getD1().prepare("SELECT id,title,description,body,image_key AS imageKey,status,author_email AS authorEmail,published_at AS publishedAt,created_at AS createdAt,updated_at AS updatedAt FROM articles WHERE status='published' ORDER BY COALESCE(published_at,created_at) DESC").all<ArticleRow>();return r.results}
export async function getPublishedArticle(id:string){return getD1().prepare("SELECT id,title,description,body,image_key AS imageKey,status,author_email AS authorEmail,published_at AS publishedAt,created_at AS createdAt,updated_at AS updatedAt FROM articles WHERE id=? AND status='published'").bind(id).first<ArticleRow>()}
